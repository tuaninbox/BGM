...
async def list_devices(
    ...
    if not has_permission(current_user.role, "read_devices", roles):
        log_action(
            current_user,
            "device_view_denied",
            f"Device View - Permission denied (read_devices)",
            request,
            category="device",
        )
        return {
    ...
    try:
        ...
        # Sorting
        # ---------------------------------------------------
        valid_sort_fields = {"name", "ip", "os", "location"}
        if sort_by not in valid_sort_fields:
            sort_by = "name"

        reverse = sort_dir == "desc"
        devices.sort(key=lambda d: d.get(sort_by, ""), reverse=reverse)
        ...
        # ---------------------------------------------------
        # Build device list with active request info
        # ---------------------------------------------------
        for dev in paged_devices:
            name = dev["name"]
        ...
        log_action(
            current_user,
            "device_view",
            f"Device View - Loaded {len(merged_devices)} devices (page {page})",
            request,
            category="device",
        )
